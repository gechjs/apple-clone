...
function Iphone() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(new URL('../assets/iphone.json', import.meta.url));
        ...
        return (
          <div key={obj.product_id}>
            <div className="row w-60">
              <div className={`col-sm-12 col-md-6 order-${x} d-flex justify-content-center align-items-center`}>
                <div>
              ...
              <div className={`col-sm-12 col-md-6 order-${y}`}>
                <img className="w-50" src={obj.product_img} alt={obj.product_name} />
              </div>
            </div>
          </div>
        );
